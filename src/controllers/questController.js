...
module.exports.checkQuestAndCharacterId = (req, res, next) => {
    if (!req.body.character_id || !req.body.quest_id) {
        res.status(400).send("Character Id or Quest Id is missing.")
        return;
    }
...
module.exports.checkAlrCompleted=(req,res,next)=>{
    const data = { character_id: req.body.character_id, quest_id: req.body.quest_id }
    const callback = (error, results, fields) => {
        if (error) {
            res.status(500).send(
            ...
            if(results.length>0){
                res.status(403).send("You have already completed this quest.")
            }
            else{
                next()
            }
    ...
    model.checkAlrCompleted(data,callback)
}
    ...
    const callback = (error, results, fields) => {
        if (error) {
            ...
            if (results[0][0].level >= results[1][0].level_requirement) {
                ...
                res.status(403).send("Level of your character doesn't meet with the level requirement of this quest.")
            }
        }
    }
